...
const LIVE_FEED = [
  {
    id: "CR-442",
    name: "Iron Ore Express",
    from: "Warsaw",
    to: "Hamburg",
    status: "ON TIME",
    time: "ETA 14:30 (Today)",
    cargo: "Raw Mineral",
  },
  {
    id: "CR-901",
    name: "Rotterdam Cement Carrier",
    from: "Rotterdam",
    to: "Genoa",
    status: "DELAYED",
    time: "ETA 19:15 (+32m)",
    cargo: "Dry Bulk Cement",
  },
  {
    id: "CR-112",
    name: "Automotive Carrier Grid A",
    from: "Munich",
    to: "Verona",
    status: "DELAYED",
    time: "ETA 22:45 (+49m)",
    cargo: "European Vehicles",
  },
  {
    id: "CR-305",
    name: "Intermodal Container Train",
    from: "Berlin",
    to: "Paris",
    status: "ON TIME",
    time: "ETA 06:15 (Tomorrow)",
    cargo: "Mixed Goods",
  },
];
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        console.log(
          "FastAPI backend offline, running on high-fidelity mock datasets.",
        );
      } finally {
        setLoading(false);
      }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            Real-time predictive logistics intelligence for Trans-European
            railway networks.
          </p>
        </div>
        <div className="flex items-center space-x-2">
          <div className="flex items-center space-x-1.5 px-3 py-1 bg-brand-blue-light dark:bg-brand-blue-light/10 border border-border-primary rounded text-[10px] font-semibold">
            <span
              className={`w-2 h-2 rounded-full ${activeSync ? "bg-emerald-500 animate-pulse" : "bg-slate-400"}`}
            ></span>
        ...
        {[
          ...
          >
            <div className="flex justify-between items-start">
              ...
              <span className="text-[9px] text-brand-green font-bold">
                {card.sub}
              </span>
            </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        <div className="xl:col-span-8 p-5 rounded-xl border border-border-primary bg-bg-card shadow-sm">
          <div className="flex justify-between items-center mb-4 border-b border-border-primary pb-2">
            <div>
              <h3 className="text-sm font-bold text-text-primary font-serif">
                Freight Delay Variance Trends
              </h3>
              <p className="text-[10px] text-slate-500">
                Aggregated predicted vs actual arrival delay across TEN-T
                European networks
              </p>
            </div>
            <div className="flex items-center space-x-2 text-[10px] font-semibold">
              <span className="px-2 py-0.5 rounded bg-brand-blue-light dark:bg-brand-blue-light/10 border border-border-primary text-brand-blue dark:text-brand-accent-blue">
                ACTUAL TIME
              </span>
              <span className="px-2 py-0.5 rounded bg-brand-saffron/10 border border-brand-saffron/20 text-[#ea580c] dark:text-[#f97316]">
                AI ESTIMATE
              </span>
            </div>
          </div>
          ...
          <div className="h-64 w-full text-slate-500 text-xs">
            {loading ? (
              <div className="h-full flex items-center justify-center text-[11px] font-semibold">
                CALCULATING CHART ARRAYS...
              </div>
            ) : (
        ...
        <div className="xl:col-span-4 p-5 rounded-xl border border-border-primary bg-bg-card shadow-sm flex flex-col justify-between">
          <div className="border-b border-border-primary pb-2 mb-3">
            <h3 className="text-sm font-bold text-text-primary font-serif">
              Live Freight Tracking Feed
            </h3>
            <p className="text-[10px] text-slate-500">
              Simulated satellite synchronization stream
            </p>
          </div>
              ...
              >
                <div className="space-y-1">
                  <div className="flex items-center space-x-1.5">
                    ...
                    <span className="font-bold text-text-primary text-[11px] font-serif">
                      {feed.name}
                    </span>
                  </div>
      ...
      <div className="p-5 rounded-xl border border-border-primary bg-bg-card shadow-sm">
        <div className="border-b border-border-primary pb-2 mb-4">
          <h3 className="text-sm font-bold text-text-primary font-serif">
            Trans-European Corridors Performance
          </h3>
          <p className="text-[10px] text-slate-500">
            Detailed efficiency logs across primary cargo transit channels
          </p>
        </div>
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-border-primary/30 text-slate-700 dark:text-slate-300">
              {corridors.map((row, i) => (
                <tr
                  key={i}
                  className="hover:bg-slate-50 dark:hover:bg-brand-blue-light/5 transition-colors"
                >
                  <td className="py-3 font-bold text-text-primary font-serif">
                    {row.corridor}
                  </td>
                  <td className="py-3 font-semibold text-brand-blue dark:text-brand-accent-blue">
                    {row.avg_delay}
                  </td>
                  <td className="py-3">
